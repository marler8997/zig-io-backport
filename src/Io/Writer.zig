...
pub fn print(w: *Writer, comptime fmt: []const u8, args: anytype) Error!void {
    const Writer2 = @import("std").io.GenericWriter(
        *Writer,
        Writer.Error,
        (struct {
            fn write(w2: *Writer, bytes: []const u8) Writer.Error!usize {
                return try w2.write(bytes);
            }
        }).write,
    );
    try std.fmt.format(Writer2{ .context = w }, fmt, args);
}
...
pub fn sendFile(w: *Writer, file_reader: *File15.Reader, limit: Limit) FileError!usize {
...
pub fn unimplementedSendFile(w: *Writer, file_reader: *File15.Reader, limit: Limit) FileError!usize {
    _ = w;
    _ = file_reader;
    _ = limit;
    return error.Unimplemented;
